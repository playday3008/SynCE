...
extern "C"
{
    void* init_libpocketpckonnector()
    {
...
//#ifdef __USE_ABOVE_KDEPIM_3_3_0__
    ...
    m_baseDir = storagePath() + "/pocketpc/"; // + calHandler.getPartnerId(); //QDir::homeDirPath() + "/.kitchensync/meta/pocketpc/" + calHandler.getPartnerId(); 
    /*
#else
    m_baseDir = QDir::homeDirPath() + "/.kitchensync/meta/pocketpc/";
#endif
    */ 
...
bool PocketPCKonnector::readSyncees()
{
    ...
    if (firstSync)
    {
        ...
        {
            m_rra->connect(); // to ensure correct connection-state!
            emit synceeReadError(this);
            return false;
        }
  
        m_rra->finalDisconnect(); // think this is necessary.. *hmpf*    
...
bool PocketPCKonnector::writeSyncees()
{
    ...
    if (m_uidHelper)
    {
        ...
        //m_uidHelper->save(); // not necessary.. cause nothing in the KonnectorUIDHelper is changed!
    }            
...
Kapabilities PocketPCKonnector::capabilities()
{
    Kapabilities caps;
    
    caps.setSupportMetaSyncing(true);
    caps.setSupportsPushSync(true); // was true...
    ...
    caps.setAutoHandle(true); // really??
    caps.setNeedAuthentication(false);
    caps.setNeedsModelName(false); 
