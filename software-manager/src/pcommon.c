...
static bool anyfile_remote_read(AnyFile* file, unsigned char* buffer, size_t bytes, size_t* bytesAccessed)
{
	bool retval;
	DWORD read_len;
	retval = CeReadFile(file->handle.remote, buffer, bytes, &read_len, NULL);
	*bytesAccessed = read_len;
	return retval;
}
...
static bool anyfile_remote_write(AnyFile* file, unsigned char* buffer, size_t bytes, size_t* bytesAccessed)
{
	bool retval;
	DWORD write_len;
	retval = CeWriteFile(file->handle.remote, buffer, bytes, &write_len, NULL);
	*bytesAccessed = write_len;
	return retval;
}
