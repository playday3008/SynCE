...
def GenerateCombinedID(folderID,messageID):
	return folderID+":"+messageID

def MessageIDFromCombinedID(combinedID):
	s=combinedID.split(":")
	if len(s)>1:
		return s[1]
	else:
...
def SplitCombinedID(combinedID):
	s=combinedID.split(":")
	if len(s)>1:
		return (s[0],s[1])
	else:
...
def GetIncrementedSyncKey(folderID,key):
	keyID,key = GetSyncKeyData(key)
	if keyID=="":
		keyID = folderID
	return folderID+str(key+1)
