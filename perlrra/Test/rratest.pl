...
# rra_get_object_ids
#my $ids=rra_get_object_ids($rra, 0x2712);
...
#my ($data, $len)=rra_object_get($rra, 0x2712, 0x16a8);
...
# rra_object_get/put (put in an existing object_id)
print "rra_object_get($rra, 0x2711, 0x1688);\n";
my ($data, $len)=rra_object_get($rra, 0x2711, 0x1688);
die "rra_object_get failed" unless defined $data;
#printf "data[%d]='%s'\n", $len, $data;

my $ve=rra_appointment_to_vevent(0, $data, $len, 0);
