...
static WBXMLEncoder *wbxml_encoder_duplicate(WBXMLEncoder *encoder);
...
WBXML_DECLARE(WBXMLError) wbxml_encoder_encode_to_wbxml(WBXMLEncoder *encoder, WB_UTINY **wbxml, WB_ULONG *wbxml_len)
{
    ...
    *wbxml_len = 0;
    ...
    encoder->output = wbxml_buffer_create("", 0, WBXML_ENCODER_WBXML_DOC_MALLOC_BLOCK);
    ...
    switch (encoder->tree->lang->langID) 
    {
#if defined( WBXML_SUPPORT_WV )
...
WBXML_DECLARE(WBXMLError) wbxml_encoder_encode_to_xml(WBXMLEncoder *encoder, WB_UTINY **xml)
{
    ...
    *xml = NULL;
    ...
    encoder->output = wbxml_buffer_create("", 0, WBXML_ENCODER_XML_DOC_MALLOC_BLOCK);
    if (encoder->output == NULL) {
        wbxml_encoder_destroy(encoder);
        return WBXML_ERROR_NOT_ENOUGH_MEMORY;
    }
    ...
    switch (node->type) {
   	case WBXML_TREE_ELEMENT_NODE:
        ...
        case WBXML_ENCODER_OUTPUT_XML:
   		/* Always add an XML End Element tag (even if not Child) */
...
{   
    ...
    case WBXML_ENCODER_OUTPUT_WBXML:
         ...
         *        tokenized at all.
         */
        if ((node->attrs != NULL) && (encoder->tree->lang->attrTable != NULL) /** @todo Fast patch for SyncML (change this) */) {
            if ((ret = wbxml_encode_end(encoder)) != WBXML_OK)
                return ret;
...
static WBXMLError parse_text(WBXMLEncoder *encoder, WBXMLTreeNode *node)
{
    ...
    /* If Canonical Form: "Ignorable white space is considered significant and is treated equivalently to data" */
	    if ((encoder->output_type != WBXML_ENCODER_OUTPUT_XML) || (encoder->xml_gen_type != WBXML_ENCODER_XML_GEN_CANONICAL)) {
        /* Ignore blank nodes */
...
{    
    ...
    header = wbxml_buffer_create("", 0, WBXML_ENCODER_WBXML_HEADER_MALLOC_BLOCK);
    if (header == NULL)
        return WBXML_ERROR_NOT_ENOUGH_MEMORY;
    ...
    if (public_id == WBXML_PUBLIC_ID_UNKNOWN) 
    {
        if (encoder->tree->lang->publicID->xmlPublicID != NULL) 
        {
            ...
            strstbl_len = encoder->strstbl_len;
#else
            /* Length of String Table is length of XML Public ID (including terminating NULL char) */
    ...
    *wbxml_len = wbxml_buffer_len(header) + strstbl_len + wbxml_buffer_len(encoder->output);
...
#else
    /* The copy includes terminating NULL char */
    memcpy(*wbxml + wbxml_buffer_len(header), wbxml_buffer_get_cstr(pid), strstbl_len);

    wbxml_buffer_destroy(pid);
#endif /* WBXML_ENCODER_USE_STRTBL */

    /* Copy WBXML Buffer */
    memcpy(*wbxml + wbxml_buffer_len(header) + strstbl_len, wbxml_buffer_get_cstr(encoder->output), wbxml_buffer_len(encoder->output));
...
static WBXMLError wbxml_encode_value_element_buffer(WBXMLEncoder *encoder, WB_UTINY *buffer, WBXMLValueElementCtx ctx)
{
    ...
    if ((ctx == WBXML_VALUE_ELEMENT_CTX_CONTENT) && (!encoder->in_cdata))
    {
        ...
        if ((encoder->tree->lang->langID == WBXML_LANG_WV_CSP11) ||
            (encoder->tree->lang->langID == WBXML_LANG_WV_CSP12))
        {
        ...
        if (encoder->tree->lang->langID == WBXML_LANG_DRMREL10)
        {
        ...
        if (encoder->tree->lang->attrValueTable != NULL) {
            ...
            while (encoder->tree->lang->attrValueTable[j].xmlName != NULL)
            {
                ...
                for (i = 0; i < wbxml_list_len(lresult); i++) {
                    ...
                    if (wbxml_buffer_search_cstr(elt->u.str, (WB_UTINY *)encoder->tree->lang->attrValueTable[j].xmlName, 0, &index)) {
        ...
        if (encoder->tree->lang->extValueTable != NULL) {
            ...
            while (encoder->tree->lang->extValueTable[j].xmlName != NULL)
            {
                ...
                for (i = 0; i < wbxml_list_len(lresult); i++) {
                    ...
                    if (wbxml_buffer_search_cstr(elt->u.str, (WB_UTINY *) encoder->tree->lang->extValueTable[j].xmlName, 0, &index)) 
                    {
                        /* Create new Value Element */
                        if ((new_elt = wbxml_value_element_create()) == NULL) {
                            wbxml_list_destroy(lresult, wbxml_value_element_destroy_item);
...
static WBXMLError wbxml_encode_tree(WBXMLEncoder *encoder, WBXMLTree *tree)
{
    ...
    if ((new_encoder = wbxml_encoder_duplicate(encoder)) == NULL)
        return WBXML_ERROR_NOT_ENOUGH_MEMORY;
    ...
    /* Encode to WBXML */
    if ((ret = wbxml_encoder_encode_to_wbxml(new_encoder, &wbxml, &wbxml_len)) != WBXML_OK) {
...
static WBXMLError xml_encode_tag(WBXMLEncoder *encoder, WBXMLTreeNode *node)
{
    ...
    if ((encoder->tree->lang->nsTable != NULL) &&
        ((node->parent == NULL) ||
    ...
    {
        if ((ns = wbxml_tables_get_xmlns(encoder->tree->lang->nsTable, node->name->u.token->wbxmlCodePage)) != NULL)
        {
            /* Append xmlns=" */
...
static WBXMLError xml_encode_tree(WBXMLEncoder *encoder, WBXMLTree *tree)
{
    ...
    /* Encode to XML */
    if ((ret = wbxml_encoder_encode_to_xml(new_encoder, &xml)) != WBXML_OK) {
        wbxml_encoder_destroy(new_encoder);
        return ret;
    }
