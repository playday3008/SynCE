...
static WBXMLError decode_wv_content(WBXMLParser *parser, WB_UTINY **data, WB_LONG *len, WB_BOOL *static_content);
...
static WBXMLError decode_drmrel_keyvalue(WB_UTINY **data, WB_LONG *len);
...
static WB_BOOL check_public_id(WBXMLParser *parser)
{
    ...
    if (parser->public_id_index != -1) {
        ...
        if (get_strtbl_reference(parser, (WB_ULONG) parser->public_id_index, &public_id, &len) != WBXML_OK) {
        ...
        while (parser->mainTable[index].publicID != NULL)
        {
            if ((parser->mainTable[index].publicID->xmlPublicID != NULL) && 
                (WBXML_STRCASECMP(parser->mainTable[index].publicID->xmlPublicID, public_id) == 0))
            {
                parser->langTable = &(parser->mainTable[index]);
...
static WBXMLError parse_element(WBXMLParser *parser)
{
    ...
    if (tag & WBXML_TOKEN_WITH_ATTRS) {
        ...
        do {
            ...
            attrs = wbxml_realloc(attrs, (attrs_nb + 1) * sizeof(*attrs));
            if (attrs == NULL) {
                /* Clean-up */
                wbxml_tag_destroy(element);
...
static WBXMLError parse_extension(WBXMLParser *parser, WBXMLTokenType code_space, WB_UTINY **ext, WB_LONG *len)
{
    ...
    *len = 0;
    ...
    case WBXML_LANG_WTAWML12:
        ...
        /* Build Variable */
        *ext = (WB_UTINY*) wbxml_malloc(WBXML_STRLEN(var_begin) + var_len + WBXML_STRLEN(escape) + WBXML_STRLEN(var_end) + 1);
        if (*ext == NULL)
            return WBXML_ERROR_NOT_ENOUGH_MEMORY;    
...
static WBXMLError parse_entity(WBXMLParser *parser, WB_UTINY **entity, WB_LONG *len)
{
    ...
    /* WARNING: If you change this variable length (10 chars), change too 'WBXML_PARSER_MAX_ENTITY_CODE' 
    ...
    *len = sprintf((WB_TINY *)(*entity), "&#%u;", code);
...
static WBXMLError parse_attr_start(WBXMLParser *parser, WBXMLAttributeName **name, WB_UTINY **value)
{
    ...
    if (parser->langTable->attrTable[index].xmlValue != NULL)
        *value = (WB_UTINY *) parser->langTable->attrTable[index].xmlValue;
...
static WBXMLError parse_attr_value(WBXMLParser *parser, WB_UTINY **value, WB_LONG *len, WB_BOOL *static_value)
{
    ...
    *static_value = TRUE;
    ...
    while ((parser->langTable->attrValueTable[index].xmlName != NULL) &&
           ((parser->langTable->attrValueTable[index].wbxmlToken != tag) ||
            (parser->langTable->attrValueTable[index].wbxmlCodePage != parser->tagCodePage))) {
        index++;
    }
    ...
    *value = (WB_UTINY *) parser->langTable->attrValueTable[index].xmlName;
...
static WBXMLError decode_opaque_content(WBXMLParser *parser, WB_UTINY **data, WB_LONG *len, WB_BOOL *static_content)
{
    ...
    /* This function is always called after 'parse_opaque()' into 'parse_content()', and
     * 'parse_opaque()' always output a static buffer.
     */
    *static_content = TRUE;
    ...
    case WBXML_LANG_WV_CSP11:
    case WBXML_LANG_WV_CSP12:
        return decode_wv_content(parser, data, len, static_content);
        break;
#endif /* WBXML_SUPPORT_WV */
    ...
    case WBXML_LANG_DRMREL10:
        ...
        {
            /* Decode <KeyValue> */
            if ((ret = decode_drmrel_keyvalue(data, len)) != WBXML_OK)
                return ret;
...
static WBXMLError decode_drmrel_keyvalue(WB_UTINY **data, WB_LONG *len)
{
    ...
    if ((result = wbxml_base64_encode((const WB_UTINY *) *data, *len)) == NULL)
