...
void wbxml_tree_clb_xml_start_element(void *ctx, const XML_Char *localName, const XML_Char **attrs)
{
    ...
    {
        if ((node->attrs = construct_attribute_list(tree_ctx->tree->lang, attrs)) == NULL) {
    ...
    if (tree_ctx->skip_lvl > 0) {
        if (tree_ctx->skip_lvl == 1)
        {
            ...
            if (WBXML_STRCMP(localName, "DevInf") == 0) {
                ...
                if ((ret = wbxml_tree_from_xml(wbxml_buffer_get_cstr(devinf_doc), &tree)) != WBXML_OK) {
                    tree_ctx->error = ret;
                    wbxml_buffer_destroy(devinf_doc);
                    return;
                }
...
text_node:
    ...
    /* Set Content */
    if ((node->content = wbxml_buffer_create(ch, len, len)) == NULL) {
        wbxml_tree_node_destroy(node);
        tree_ctx->error = WBXML_ERROR_NOT_ENOUGH_MEMORY;
        return;
    }
